...
！同理：如果用反事实变量来描述这个问题，答案变得显而易见。假设对一个当前水平为X*的变量X增加数量q，结果将会变成了。这个结果在当前水平X = x″的所有个体中的平均值是E(K,1 x″)，其中，=’ + 4。这里再次遇到ETT表达式E(Y, l x)。我们可以将上节实例的方法应用到这个实例上，特别地，可以立刻得到结论：只要模型中的某个变量集合Z满足后门准则，那么添加干预的效应就可以用式（4.21）的ETT校正公式来估计。在式（4.21）中执行替换 = x′ + g 并取期望，可以得到这个干预的效应，我们称之为add(g)：
