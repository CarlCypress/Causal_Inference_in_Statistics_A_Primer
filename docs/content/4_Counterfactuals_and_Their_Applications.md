...
！毋庸置疑的是，无论是在观察研究中还是在试验研究中，都不能完整获得像表4.3这样的数据。这些数据是从诸如图4.2的参数化模型中推导出来的。从该模型中，能够在给定观察X以及其他引导条件下，推导每一个参与者的属性U、U₀、U₁等。一般而言，如果缺少参数化模型，仅仅观察他们的行为（X、Y、H等），我们几乎无法知道任何个体参与者的潜在结果（如Y₀和Y₁）。从理论上讲，我们仅有的反事实Y与观察数据（X, Y, H）之间的联系，是公式（4.6）中的一致性原则。该原则说明：当X=1时，Y一定等于Y₁；当X=0时，Y一定等于Y₀。除了这个简单的联系外，与个体参与者有关的大部分反事实将无法被观测。
