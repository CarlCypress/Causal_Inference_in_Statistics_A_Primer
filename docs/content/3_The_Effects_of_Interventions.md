...
!与基于条件概率的定义相比，这一定义的显著优势是其通用性；它剥离了“保持不变”的含义，即使Z-Y的关系是混杂的情况（同样适用于X-Z和X-Y的关系是混杂的情况），实际上，这个定义保证了，在任何预估概率可由观测概率确定的情况下，都能估计X对Y的直接效应。注意：对于不同的X值，直接效应可能会不同；例如，对于资质要求高的工作，雇佣政策歧视女性；而对于资质要求低的工作，雇佣政策歧视男性。因此，想要获得直接效应的全部信息，必须对Z的每一个相关取值进行计算（在线性模型中，则不必这么做，详见3.8节）。
